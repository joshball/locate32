/* Copyright (c) 1997-2007 Janne Huttunen
   locate.exe v3.0.7.9220                 */

const char* szVersionStr="locate 3.0 build 7.9220";
	...
	for (i=1;i<argc;i++)
	{
		if ((argv[i][0]==L'-' || argv[i][0]==L'/') && !options.noargs)
		{
			...
			case L'L':
			{
				...
				case L'm':
            		if (argv[i][3]==L':')
            			ulMinSize=_wcstoui64(argv[i]+4,&ep,0);
            		else
            			ulMinSize=_wcstoui64(argv[i]+3,&ep,0);
                    if (*ep==L'k' || *ep==L'K')
                       ulMinSize*=1024;
            	...
            	case L'M':
            		if (argv[i][3]==L':')
            			ulMaxSize=_wcstoui64(argv[i]+4,&ep,0);
            		else
            			ulMaxSize=_wcstoui64(argv[i]+3,&ep,0);
                    if (*ep==L'k' || *ep==L'K')
                       ulMaxSize*=1024;
                    else if (*ep==L'M' || *ep==L'm')
                         ulMaxSize*=1024*1024;
